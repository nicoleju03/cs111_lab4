...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group  = BLOCK_SIZE*8; // (size of bitmap in bytes) *( number of bits in a byte)
...
void write_block_bitmap(int fd) {
	...
	bitmap[127]=0x80;
	//filesystem only has 1024 blocks so mark the non-existent blocks as in use
...
void write_inode_table(int fd) {
	...
	write_inode(fd, 2, &root_dir_inode);
...
void write_root_dir_block(int fd) {
	...
	dir_entry_set(parent_entry, EXT2_ROOT_INO, ".."); //not sure
